...
contract('TestSquareVerifier', accounts => {
    ...
    describe('verifier works as expected', function () {
        ...
        it('correct proof verify', async function () { 
            
            //get the parameters from the correct proof
            ...
            //call verifyTX to verify to proof (returns bool)
            let returnedValue = await this.contract.verifyTx.call(a, a_p, b, b_p, c, c_p, h, k, input);
            //console.log(returnedValue);
            assert(returnedValue, 'Verification should return true!');
        })
        
        // Test verification with incorrect proof
        it('wrong proof does NOT verify', async function () { 
            let a = wrongProof.proof.A;
            let a_p = wrongProof.proof.A_p;
            let b = wrongProof.proof.B;
            let b_p = wrongProof.proof.B_p;
            let c = wrongProof.proof.C;
            let c_p = wrongProof.proof.C_p;
            let h = wrongProof.proof.H;
            let k = wrongProof.proof.K;
            let input = wrongProof.input;
            //call verifyTx to verify to proof (returns bool)
            let returnedValue = await this.contract.verifyTx.call(a, a_p, b, b_p, c, c_p, h, k, input);
            //console.log(returnedValue);
            assert(returnedValue, false, 'Verification should return false!');
        })
    });
});
